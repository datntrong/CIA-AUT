    ...
    public String getInputForGoogleTest(boolean isDeclared) throws Exception {
        ...
        if(!isPassingVariable() || (isPassingVariable() && isDeclared) ) {
            ...
            if (this.getChildren().size() > 0) {
                ...
                String randomName = "autRandomName" + new RandomDataGenerator().nextInt(0, 999999);
                String elementType = getStringToCharacterTypeMap().get(VariableTypeUtils.removeRedundantKeyword(typeVar));
...
//                    init += String.format("%s[%s] = '%s';", randomName, i, v, "");
                ...
                if (isPassingVariable()) {
                    input += typeVar + SpecialCharacter.SPACE + getVirtualName() + SpecialCharacter.EQUAL  + value + SpecialCharacter.END_OF_STATEMENT;

                } else if (isAttribute()) {
                    input += getVirtualName() + SpecialCharacter.EQUAL + value + SpecialCharacter.END_OF_STATEMENT;
