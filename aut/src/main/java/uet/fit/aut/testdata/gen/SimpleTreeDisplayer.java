    ...
    private void displayTree(IDataNode node, int level){
        if (node == null) {
            ...
            if (node instanceof RootDataNode)
                ...
                treeInString += genTab(level + 1) + "virtual name: " + n.getVirtualName() + "\n";
                ...
                if (n.getParent() != null)
                    treeInString += genTab(level + 1) + "virtual name of parent: " + n.getParent().getVirtualName() + "\n";

                if (n instanceof ValueDataNode) {
                    treeInString += genTab(level + 1) + "is external variable : " + ((ValueDataNode) n).isExternal() + "\n";
                    treeInString += genTab(level + 1) + "type: " + ((ValueDataNode) n).getRawType() + "\n";
                    ...
                    try {
                        treeInString += genTab(level + 1) + "script: " + n.getInputForGoogleTest(false).replace("\n", "") + "\n";
                    } catch (Exception e) {
