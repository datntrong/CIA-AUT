...
import uet.fit.aut.search.condition.*;
...
public class CTypeDependencyGeneration extends AbstractDependencyGeneration<IVariableNode> {

//    private static final Logger logger = LoggerFactory.getLogger(CTypeDependencyGeneration.class);
    private static final Logger logger = new NOPLogger() {};

    private final Map<String, INode> sCacheResolvedNodes = new HashMap<>();
    ...
    public CTypeDependencyGeneration() {
        super();
    }

    public void dependencyGeneration(IVariableNode root) {
        IVariableNode variableNode = root;
        IVariableNode resolvedVarNode = root;
        ...
        if (correspondingNode != null) {
            ...
            if (!resolvedVarNode.isTypeDependencyState()) {
                ...
                try {
                    ...
                    if (correspondingNode == null && TemplateUtils.isTemplate(realType))
                        correspondingNode = performTemplateSearch(spaces, resolvedVarNode);

                    if (correspondingNode == null) {
                        correspondingNode = performExtendSpace(spaces, resolvedVarNode);
                    }
    ...
    private INode performExtendSpace(List<Level> spaces, IVariableNode resolvedVarNode) throws Exception {
        Level lastLevel = spaces.get(spaces.size() - 1);

        for (INode n : lastLevel) {
            if (n instanceof ISourcecodeFileNode) {
                List<Level> levels = new VariableSearchingSpace(n).generateExtendSpaces();

                if (correspondingNode == null)
                    correspondingNode = performComplexSearch(levels, resolvedVarNode);

                if (correspondingNode == null)
                    correspondingNode = performScopeSearch(levels, resolvedVarNode);
    ...
    private void fetchRelatedFiles(List<ISourcecodeFileNode> sourceNodes, ISourcecodeFileNode sourceNode) {
        List<Level> space = new VariableSearchingSpace(sourceNode).generateExtendSpaces();
        List<ISourcecodeFileNode> sources = Search.searchInSpace(space, new SourcecodeFileNodeCondition());
        for (ISourcecodeFileNode s : sources) {
            if (!sourceNodes.contains(s)) {
                sourceNodes.add(s);
                fetchRelatedFiles(sourceNodes, s);
            }
        }
    }
    ...
    private INode searchExistTypeDependency(IVariableNode variableNode) {
        ...
        if (parent != null && parent.getAbsolutePath() != null && !parent.getAbsolutePath().isEmpty()) {
            ...
            correspondingNode = sCacheResolvedNodes.get(key);
        }
    ...
    private void saveTypeDependency(IVariableNode variableNode, INode correspondingNode) {
        ...
        if (parent != null && parent.getAbsolutePath() != null && parent.getAbsolutePath().length() > 0) {
            ...
            if (!sCacheResolvedNodes.containsKey(key))
                sCacheResolvedNodes.put(key, correspondingNode);
        }

        variableNode.setCorrespondingNode(correspondingNode);
    }
    ...
    private INode performComplexSearch(List<Level> spaces, IVariableNode node) throws Exception {
        ...
        if (candidateNodes.size() == 1) {
            ...
            if (currentNode instanceof StructOrClassNode) {
                ...
                List<List<INode>> extendPaths = ((NamespaceNode) currentNode).getExtendPaths();

                for (List<INode> path : extendPaths) {
                    String pathInString = getExtendPathInString(path);
            ...
            if (type.startsWith(SpecialCharacter.STRUCTURE_OR_NAMESPACE_ACCESS)) {
                for (INode candidateNode : candidateNodes)
                    if (candidateNode.getParent() instanceof ISourcecodeFileNode)
                ...
                if (tmp.size() == 1)
                    ...
                    throw new Exception("Don't found the matching node with " + type);
                else {
    ...
    private String getExtendPathInString(List<INode> path) {
            ...
            if (ast instanceof IASTDeclarationStatement) {
                ...
                if (ast instanceof IASTSimpleDeclaration) {
                    ...
                    if (declarator instanceof IASTFunctionDeclarator) {
                        ...
                        typeNode.setParent(resolvedVarNode);

                        String returnType = ASTVisualizer.toString(declaration.getDeclSpecifier());
                        typeNode.setReturnType(returnType);

                        String functionName = declarator.getNestedDeclarator().getName().toString();
                        ...
                        for (int i = 0; i < params.length; i++) {
                            paramTypes[i] = ASTVisualizer.toString(params[i].getDeclSpecifier());
                        }
