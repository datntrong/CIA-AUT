...
public abstract class StructOrClassNode extends StructureNode {
    ...
    private List<String> getExtendNames() {
        ...
        if (d1 instanceof ICPPASTCompositeTypeSpecifier)
            for (ICPPASTCompositeTypeSpecifier.ICPPASTBaseSpecifier b : ((ICPPASTCompositeTypeSpecifier) d1).getBaseSpecifiers())
                output.add(b.getNameSpecifier().toString());
    ...
    private void getExtendPaths(INode n, ArrayList<INode> path) {
        ...
        if (extendedNodes.size() > 0)
            ...
            extendPaths.add(new ArrayList<>(path));
     ...
     * @return
     */
    public List<IVariableNode> getAllAttributes() {
        List<IVariableNode> attributes = getAttributes();
    ...
    public boolean isAbstract() {
        for (INode child : getChildren()) {
            if (child instanceof DefinitionFunctionNode) {
                DefinitionFunctionNode functionNode = (DefinitionFunctionNode) child;
                if (functionNode.getSingleSimpleName().startsWith("~"))
                    continue;
                String code = functionNode.getAST().getRawSignature();
                if (code.contains("virtual ")) {
                    return true;
                }
            } else if (child instanceof AbstractFunctionNode && !(child instanceof DestructorNode)) {
                String code = ((AbstractFunctionNode) child).getAST().getRawSignature();
                if (code.contains("virtual ")) {
                    return true;
                }
            }
