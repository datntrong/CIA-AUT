...
public class FunctionPointerTypedefDeclaration extends TypedefDeclaration implements IFunctionPointerTypeNode {
    ...
    private IASTName refNameAst; // "ListCompareFunc"
    ...
    public String getNewType() {
        return refNameAst.toString(); // "ListCompareFunc"
    }

    @Override
    public String getOldType() {
        String returnType = ASTVisualizer.toString(returnAst);

        IASTDeclarator declarator = getAST().getDeclarators()[0].getNestedDeclarator();
        ...
        if (declarator instanceof IASTArrayDeclarator) {
            ...
            for (IASTArrayModifier modifier : modifiers) {
                name += "[" + ASTVisualizer.toString(modifier.getConstantExpression()) + "]";
            }
        }
        ...
        String oldType = returnType + " " + "(" + prefix + name + ")" + "(";
        for (IVariableNode arg : getArguments()) {
            oldType += arg.getRawType();
            if (!arg.getName().isEmpty())
                oldType += " " + arg.getName();
            oldType += ", ";
        }
        oldType += ")";
        return oldType.replace(", )", ")");
//        return getAST()
//                .getRawSignature()
...
//                .replaceFirst("^typedef\\s+", SpecialCharacter.EMPTY);
    ...
    public String getReturnType() {
        String returnType = ASTVisualizer.toString(returnAst, getAST().getDeclarators()[0]);
