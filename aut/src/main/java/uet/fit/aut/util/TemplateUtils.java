...
public class TemplateUtils {
    ...
    public static boolean isTemplateTypeDefinedByUser(String rawType, ICommonFunctionNode fn) {
        ...
        if (functionNode.isTemplate()) {
            ...
            if (functionNode.getAST() instanceof ICPPASTTemplateDeclaration) {
                templateDeclaration = (ICPPASTTemplateDeclaration) functionNode.getAST();
            } else if (functionNode.getAST().getParent() instanceof ICPPASTTemplateDeclaration) {
                templateDeclaration = (ICPPASTTemplateDeclaration) functionNode.getAST().getParent();
            }
            ...
            if (templateDeclaration != null){
                ...
                for (ICPPASTTemplateParameter parameter: parameters) {
                    // get typename"T"
                    String typeName = ((ICPPASTSimpleTypeTemplateParameter) parameter).getName().getRawSignature();
//                    if (typeName.equals(rawType)){
    ...
    public static String getTemplateFullRawType(VariableNode variableNode) {
        ...
        List<SearchCondition> conditions = Arrays.asList(
                new AbstractFunctionNodeCondition(),
                new DefinitionFunctionNodeCondition()
        );

        List<INode> functions = Search
                .searchNodes(Utils.getRoot(variableNode), conditions);

        for (INode function : functions) {
            if (PathUtils.equals(function.getAbsolutePath(), functionPath)) {
                List<Level> space = new VariableSearchingSpace(function).generateExtendSpaces();
                rawType = recursiveGetFullRawType(rawType, space);
                break;
            }
        }
     ...
     */
    public static List<ICPPASTTemplateId> getAllTemplateId(IASTNode ast){
        // get all template ids
        List<ICPPASTTemplateId> templateIds = new ArrayList<>();
        ASTVisitor visitor = new ASTVisitor() {

            @Override
            public int visit(IASTExpression name) {
                // Handle case 'template1<int>(3, 7);'
                if (name.getChildren().length > 0 && name.getChildren()[0] instanceof ICPPASTTemplateId)
                    templateIds.add((ICPPASTTemplateId) name.getChildren()[0]);
            ...
            public int visit(IASTDeclaration name) {
                ...
                if (name instanceof IASTSimpleDeclaration && name.getChildren().length > 0) {
                    IASTNode firstChild = name.getChildren()[0];
                    if (firstChild instanceof IASTNamedTypeSpecifier) {
                        if (firstChild.getChildren().length > 0 && firstChild.getChildren()[0] instanceof ICPPASTTemplateId)
                            templateIds.add((ICPPASTTemplateId) firstChild.getChildren()[0]);
                    }
                }
                return ASTVisitor.PROCESS_CONTINUE;
            }
...
//            return possibleTemplates;
//        List<ICPPASTTemplateId> declarations = getAllTemplateId(Utils.getSourcecodeFile(functionNode).getAST());
//
//        //
//        for (ICPPASTTemplateId templateId : declarations) {
//            // Ex: "mypair<int>"
...
//
//                                        String paramType = new NewTypeResolver(callee, 0).exec(arguments[i]);
...
//    }
    ...
    public static List<String> getTypesInTemplate(IFunctionNode functionNode) {
        ...
        for (IASTNode searchNode : searchNodes) {
            ...
            while (ast != null && !(ast instanceof ICPPASTTemplateDeclaration) && limit > 0) {
                ast = ast.getParent();
                limit--;
            }

            if (ast != null && ast instanceof ICPPASTTemplateDeclaration) {
                ICPPASTTemplateParameter[] templateParameters = ((ICPPASTTemplateDeclaration) ast).getTemplateParameters();
                for (ICPPASTTemplateParameter templateParameter : templateParameters)
                    if (templateParameter instanceof ICPPASTSimpleTypeTemplateParameter) {
                        String templateType = ((ICPPASTSimpleTypeTemplateParameter) templateParameter).getName().getRawSignature();
    ...
    public static String getTemplateFullName(VariableNode variable) {
        ...
        List<Level> space = new VariableSearchingSpace(variable).getSpaces();

        List<INode> functions = Search.searchInSpace(space, new FunctionNodeCondition(), functionPath);

        if (functions.isEmpty())
            functions = Search.searchInSpace(space, new DefinitionFunctionNodeCondition(), functionPath);

        if (!functions.isEmpty()) {
            String classRelativePath = fullName.replace(SpecialCharacter.STRUCTURE_OR_NAMESPACE_ACCESS, File.separator);

            space = new VariableSearchingSpace(functions.get(0)).generateExtendSpaces();
        ...
        if (declSpec instanceof CPPASTCompositeTypeSpecifier) {
            ...
            for (ICPPASTCompositeTypeSpecifier.ICPPASTBaseSpecifier baseSpec : baseSpecs) {
                ...
                if (nameSpec instanceof ICPPASTTemplateId) {
                    ...
                    baseName = VariableTypeUtils.getSimpleRealType(baseName);
