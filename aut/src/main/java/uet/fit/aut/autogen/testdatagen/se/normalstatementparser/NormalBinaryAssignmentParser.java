	...
	public void parse(IASTNode ast, VariableNodeTable table, FunctionCallTable callTable) throws Exception {
		...
		if (ast instanceof IASTBinaryExpression) {
			...
			if (left instanceof ICPPASTArraySubscriptExpression)
				new ArrayItemToExpressionParser().parse(ast, table, callTable);
			else
			/*
			...
			if (left instanceof CPPASTIdExpression) {
				...
				if (right != null) {
					...
					IASTNode newAst = Utils.convertToIAST(left.getRawSignature() + " = " + right.getRawSignature());
					...
					if (initialization != null)
						for (int i = 0; i < initialization.getChildren().length; i++) {
							...
							if (child instanceof ICPPASTInitializerClause) {
								...
								new ArrayItemToExpressionParser().parse(Utils.convertToIAST(newAssignment), table, callTable);
							}
						}
				}
			} else
			...
			if (left instanceof CPPASTUnaryExpression)
				new PointerItemToExpressionParser().parse(ast, table, callTable);
			else
			/*
