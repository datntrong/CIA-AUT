...
import uet.fit.aut.autogen.testdatagen.se.memory.array.multiple_dims.MultipleDimensionSymbolicVariable;
import uet.fit.aut.autogen.testdatagen.se.memory.array.one_dim.OneDimensionSymbolicVariable;
...
import uet.fit.aut.autogen.testdatagen.se.memory.structure.SimpleStructureSymbolicVariable;
import uet.fit.aut.autogen.testdatagen.se.memory.structure.UnionSymbolicVariable;
...
import uet.fit.aut.autogen.testdatagen.testdatainit.VariablesSize;
import uet.fit.aut.autogen.testdatagen.testdatainit.VariablesSize.BASIC;
import uet.fit.aut.util.ASTUtils;
import uet.fit.aut.util.RegexUtils;
import uet.fit.aut.parser.finder.Level;
import uet.fit.aut.parser.finder.VariableSearchingSpace;
import uet.fit.aut.parser.normalizer.ClassvsStructNormalizer;
...
//		expression = ExpressionRewriterUtils.convertNullToZero(expression);
        ...
        expression = replaceEnum(expression, table.getFunctionNode());
	...
	private static String replaceEnum(String newConstraint, ICommonFunctionNode fn) {
        //replace enum with normal index of array
        List<String[]> enumItemList = new ArrayList<>();
        //find all enum related to containing file
        List<Level> dependentFileLevel = new VariableSearchingSpace(fn).generateExtendSpaces();
		List<EnumNode> enumNodes = Search.searchInSpace(dependentFileLevel, new EnumNodeCondition());
