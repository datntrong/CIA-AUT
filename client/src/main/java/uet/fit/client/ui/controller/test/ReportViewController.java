...
public class ReportViewController {
	...
	@FXML
	private ScrollPane spReport;

	public void loadContentToReportView(String content) {
		final WebView report = new WebView();
		final WebEngine webEngine = report.getEngine();
		...
		spReport.setContent(report);

		spReport.widthProperty().addListener((observable, oldValue, newValue) -> {
			Double width = (Double) newValue;
			report.setPrefWidth(width);
		});
		spReport.heightProperty().addListener((observable, oldValue, newValue) -> {
			Double height = (Double) newValue;
			report.setPrefHeight(height);
		});
	}
